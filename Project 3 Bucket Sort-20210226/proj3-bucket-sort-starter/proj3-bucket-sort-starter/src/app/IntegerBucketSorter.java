...
public class IntegerBucketSorter implements Sorter {
   ...
   */
   public int[] sort(int[] dataArray) throws Exception {
      ...
      int lenData = dataArray.length;
      buckets = new int[10][lenData];
      resetBucketValues();


      int max = findMaxIntLength(dataArray);

      for ( int placeNum = 1; placeNum <= max; placeNum++){

         distribute(dataArray, placeNum);
         collect(dataArray);
         resetBucketValues();
      }
   ...
   public void distribute(int[] dataArray, int curPlace){
      ...
      for ( int i = 0; i < dataArray.length; i++){         
         ...
         rem = getPlaceValue(curPlace, num);
         while(true){
            if ( buckets[rem][x] == -1){
               buckets[rem][x] = dataArray[i];
               break;
            }
            x++;
   ...
   */   
   ...
   //TODO4: Implement this method
         ...
         for ( int i = 0; i < array.length; i++){         
            int find = findIntLength(array[i]);  
